        ...
        <>
            ...
            {store.tiendas && store.tiendas.map((tienda, index) => <div className="card mx-auto mb-2" style={{ width: "16rem" }} key={index}>
                <div className="card-body">
                    ...
                    <div>
                    <Link to={"/profiletienda/" + tienda.id} className="btn btn-primary mt-1 mx-1">Ver tienda</Link>
                        <button className="btn btn-primary mt-4 mx-1 btnfav" onClick={() => actions.addFav(tienda.storename)}>Fav ♥</button>
                    </div>
                </div>
            </div>)}
        </>
